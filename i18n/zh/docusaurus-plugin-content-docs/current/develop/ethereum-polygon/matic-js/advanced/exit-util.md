...
description: '使用 ExitUtil 类生成证明。'
---

`matic.js`内部使用，`ExitUtil`用于生成证明。它是一个具有不同方法的类，用于帮助退出应用。

## buildPayloadForExit {#buildpayloadforexit}

它公开了`buildPayloadForExit`可用于生成证明的方法。
...
### 使用桥接客户端来生成证明 {#generating-proof-using-bridge-client}

包括 **POSClient**、**PlasmaClient** 在内的每一个桥接客户端都公开了`exitUtil`属性。
